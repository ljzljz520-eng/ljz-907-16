...
- **填写外链地址**：可先点击「检测可访问性」预检，保存时服务端再次校验：
  - 仅允许公网可访问的 `http/https` 地址（阻止内网/回环地址及重定向绕过，防 SSRF）；
  - 校验 HTTP 状态码、`Content-Type`，并读取文件头确认为真实图片；
  - 依据 `Content-Length`（缺省时实际读取）限制 5MB。
- **使用记录与找回**：每次替换都会在 `posters` 表保留旧记录（含来源、尺寸、替换时间、备注），旧本地文件**不会立即删除**；可在「使用记录」中一键重新启用任意历史海报。
